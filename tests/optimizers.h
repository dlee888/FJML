...
TEST_CASE("Test optimizers in 1d", "[optimizers]") {
    Tensor yhat = Tensor::array({0.9, 0.6, 0.75}), y = Tensor::array(std::vector<double>{1, 0, 1});
...
TEST_CASE("Testing optimizers in 2d", "[optimizers]") {
    Tensor yhat = Tensor::array(std::vector<std::vector<double>>{{0.9, 0.6}, {0.75, 0.5}}),
           y = Tensor::array(std::vector<std::vector<double>>{{1, 0}, {0.3, 0.6}});
