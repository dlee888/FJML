...
TEST_CASE("Test metrics", "[metrics]") {
    SECTION("Test accuracy") {
        Tensor labels = Tensor::array(std::vector<std::vector<float>>{{1, 0, 0}, {0, 1, 0}, {0, 0, 1}});
        Tensor predictions =
            Tensor::array(std::vector<std::vector<float>>{{1, 0, 0}, {0.1, 0.8, 0.1}, {0.6, 0.3, 0.1}});

        REQUIRE(MLP::accuracy.compute(labels, predictions) == Approx(2.0 / 3.0));
    }

    SECTION("Test sparse accuracy") {
        Tensor labels = Tensor::array(std::vector<float>{0, 1, 2});
        Tensor predictions =
            Tensor::array(std::vector<std::vector<float>>{{1, 0, 0}, {0.1, 0.8, 0.1}, {0.6, 0.3, 0.1}});
